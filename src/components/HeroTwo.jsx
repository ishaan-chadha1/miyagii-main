...
function HeroTwo() {
  // State to hold the current word
  const [currentWord, setCurrentWord] = useState("MIYAGI");
  // Array of words for "Miyagi" in different languages
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      setCurrentWord(prevWord => {
        const currentIndex = miyagiWords.indexOf(prevWord);
        const nextIndex = (currentIndex + 1) % miyagiWords.length;
        return miyagiWords[nextIndex];
      });
        ...
        <div>
          <h1 className={`${styles.heroHead2Text} text-white uppercase text-center`}>
            <span className="text-black">
              {"<"}With <span className="text-[#5379FE]">{currentWord}</span> your<br />
              documents are safe{">"}
            </span>
          </h1>
        </div>
